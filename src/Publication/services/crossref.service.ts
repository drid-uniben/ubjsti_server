...
class CrossrefService {
  ...
  generateDOI(article: IArticle, volume: any, issue: any): string {
    ...
    return `${this.doiPrefix}/ubjsti.${year}.${volume.volumeNumber}.${issue.issueNumber}.${sequenceNumber}`;
  }
  ...
  private buildCrossrefXML(
    ...
    const batchId = `ubjsti_${Date.now()}`;
...
<doi_batch xmlns="http://www.crossref.org/schema/5.3.1"
  ...
  <body>
    <journal>
      <journal_metadata>
        <full_title>UNIBEN Journal of Science, Technology and Innovation</full_title>
        <abbrev_title>UBJSTI</abbrev_title>
  ...
  async registerDOI(
    ...
    try {
      ...
      const batchId = batchIdMatch ? batchIdMatch[1] : `ubjsti_${Date.now()}`;
  ...
  async checkDOIStatus(doi: string): Promise<any> {
    try {
      const response = await axios.get(
        ...
        {
          headers: {
            'User-Agent': `UNIBEN-Journal-Science, Technology and Innovation/1.0 (mailto:${
              this.depositorEmail
            })`,
          },
        }
